...
#
# hip_main.dat is downloaded uncompressed directly — the .gz version on CDS
# is unreliable and frequently returns a truncated file.
# hip_main.dat is never deleted so regeneration works without internet.
# cedar-solve's tetra3 resolves all file paths relative to its own package
# directory, not ~/Solver — so hip_main.dat and the database must be copied
# there after download/generation.
# ---------------------------------------------------------------------------
echo ""
echo "[6c] Checking Hipparcos star catalogue..."
HIP_DIR="$EFINDER_HOME/Solver"
if [ ! -f "$HIP_DIR/hip_main.dat" ]; then
    echo "  Downloading Hipparcos catalogue (uncompressed, ~51MB)..."
...
# Copy hip_main.dat to tetra3 package directory unconditionally —
# required whether the file was just downloaded or already existed.
TETRA3_PKG=$("$VENV/bin/python3" -c \
    "import tetra3, os; print(os.path.dirname(tetra3.__file__))")
if [ ! -f "$TETRA3_PKG/hip_main.dat" ]; then
    cp "$HIP_DIR/hip_main.dat" "$TETRA3_PKG/hip_main.dat"
    echo "  hip_main.dat copied to tetra3 package directory."
fi
